...
export class EditGovernrateComponent implements OnInit,OnDestroy {
  ...
  ngOnInit(): void {
    this.subscription1=this.gov.getGovernerteById(Number(localStorage.getItem("govIdEdited"))).subscribe((res:IGovernrate) =>this.value.name=res.name);
    this.value.id=Number(localStorage.getItem("govIdEdited"));
  }

  update(){
    this.subscription2=this.gov.editGov(Number(localStorage.getItem("govIdEdited")),this.value).subscribe(res=>res);
